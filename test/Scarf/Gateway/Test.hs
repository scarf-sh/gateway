...
test_gateway_manifest =
  gatewayTestCase
    ...
    $ \request -> do
      ...
      (response5, capture) <- request "ctl.flyte.org" "/install"
      assertRedirect "https://raw.githubusercontent.com/flyteorg/flytectl/master/install.sh" response5
      liftIO $
        capture
          @?= Just
            ( FlatfileCapture
                { fileAbsoluteUrl = "https://raw.githubusercontent.com/flyteorg/flytectl/master/install.sh",
                  fileVariables = HashMap.empty,
